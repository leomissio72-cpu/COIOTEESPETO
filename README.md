    ...
    <div class="modal-content" role="dialog" aria-modal="true">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
            ...
            <div style="display:flex;gap:8px">
                <button class="btn btn-primary" id="salvar-fechar-comanda">Salvar/Fechar</button>
    ...
    comanda.itens.forEach((it, idx)=>{
        ...
        div.innerHTML = `<div><strong>${it.nome}</strong><div style="font-size:12px;color:#666">R$ ${it.preco.toFixed(2)} • Qtde: ${it.quantidade}</div></div>
            <div style="display:flex;gap:6px">
                ...
                <button class="btn btn-danger" onclick="removerItemComanda(${index},${idx})">Remover</button>
            </div>`;
        container.appendChild(div);
    });
...
function fecharComanda(){
    ...
    const venda = { comandaId: comanda.id, cliente: comanda.cliente, itens: comanda.itens, total, desconto:0, totalFinal: total, formaPagamento: formaPagamento||'dinheiro', data: new Date().toISOString() };
    vendas.push(venda);
...
function gerarNotaFiscal(venda){
    ...
    let html = `<div style="padding:8px"><h3>COIOTE BBQ — NOTA FISCAL</h3><p><strong>Data:</strong> ${data}</p><p><strong>Comanda:</strong> #${venda.comandaId}</p><p><strong>Cliente:</strong> ${venda.cliente||'Consumidor'}</p><hr>`;
    venda.itens.forEach(i => { html += `<div style="display:flex;justify-content:space-between"><div>${i.quantidade}x ${i.nome}</div><div>R$ ${(i.preco*i.quantidade).toFixed(2)}</div></div>`});
    html += `<hr><div style="display:flex;justify-content:space-between;font-weight:700"><div>TOTAL</div><div>R$ ${venda.totalFinal.toFixed(2)}</div></div></div>`;
...
function carregarRelatorios(){
    ...
    if (!vendas.length){ tbody.innerHTML = '<tr><td colspan="5" style="text-align:center">Nenhuma venda</td></tr>'; return; }
    vendas.forEach((v,idx)=>{
        const tr = document.createElement('tr');
        tr.innerHTML = `<td>${new Date(v.data).toLocaleString()}</td><td>${v.comandaId}</td><td>${v.cliente||'Consumidor'}</td><td>R$ ${v.totalFinal.toFixed(2)}</td>
            <td><button class="btn btn-primary" onclick="gerarNotaFiscalIndex(${idx})">Ver</button></td>`;
        tbody.appendChild(tr);
    });
}
...
function carregarNotasFiscais(){
    ...
    vendas.forEach(v=>{
        const div = document.createElement('div'); div.className='card';
        div.innerHTML = `<div style="display:flex;justify-content:space-between"><div>Comanda #${v.comandaId} - ${v.cliente}</div><div>R$ ${v.totalFinal.toFixed(2)}</div></div>`;
        container.appendChild(div);
    });
}
...
function atualizarDashboard(){
    ...
    comandas.forEach((c, idx) => {
        ...
        el.innerHTML = `<div style="display:flex;justify-content:space-between"><div><strong>#${c.id}</strong> ${c.cliente||''}</div><div>R$ ${total.toFixed(2)}</div></div>
                        <div style="margin-top:6px"><button class="btn btn-primary" onclick="abrirModalComanda(${idx})">Abrir</button></div>`;
...
(function init(){
    ...
    document.querySelectorAll('.payment-option').forEach(el=>{
        ...
        el.addEventListener('click', ()=>{
            document.querySelectorAll('.payment-option').forEach(x=>x.style.border='1px solid #ddd');
            el.style.border = '2px solid var(--secondary)';
            formaPagamento = el.dataset.payment;
        });
    });

    // aplicar filtros (simples: atualiza dashboard)
